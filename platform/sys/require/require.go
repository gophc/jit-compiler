...
func EqualErrno(t TestingT, expected Errno, err error, formatWithArgs ...interface{}) {
	if err == nil {
	...
	if se, ok := err.(Errno); !ok {
		fail(t, fmt.Sprintf("expected %v to be a Errno", err), "", formatWithArgs...)
	} else if se != expected {
...
// NoError fails if to err is not nil.
//
//   - formatWithArgs are optional. When the first is a string that contains '%', it is treated like fmt.Sprintf.
...
func failStack() (fs []string) {
	for i := 0; ; i++ {
		...
		// Ensure we don't add functions in the requirement package to the failure stack.
		dir := path.Dir(file)
